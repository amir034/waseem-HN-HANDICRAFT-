...
async function getBlobText(pathname) {
  ...
  try {
    ...
    const response = await get(meta.url, { token });
...
async function loadStore() {
  ...
  if (redis) {
    ...
    if (data && data.users) sources.push(data.users);
  }

  const blobData = await loadFromBlob();
  if (blobData?.users?.length) sources.push(blobData.users);
  ...
  const users = mergeUserLists(...sources);
  return { users };
}
